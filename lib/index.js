...
export { scanSecrets, writeMemory, approveSuggestion, rejectSuggestion, archiveSuggestion, sweepSuggestions, relate, similarity } from './writer.js'
...
export function apply(ctx, config = {}) {
  ...
  try {
    ctx.on?.('session/event', (subject, event) => {
      try {
        ...
        if (!id || !event) return
        const state4 = state.live.get(id) ?? { lastSeen: 0, seq: 0, turns: 0 }
  ...
  const runtime = {
    ...
    approveSuggestion({ id, overrides = {}, decidedBy = 'user' }) {
      ...
      return writerModule.approveSuggestion({ store: state.store, id, overrides, decidedBy })
    },
