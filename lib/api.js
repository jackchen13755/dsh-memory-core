      ...
      const handler = async (req, res) => {
        ...
        try {
          ...
          if (method === 'GET' && path === '/suggestions') {
            ...
            const rows = store.listSuggestions({ status, kind, limit: Number(url.searchParams.get('limit') ?? 100) })
            return sendJson(res, 200, {
              entries: rows.map((r) => {
                let payload = {}
                try {
                ...
                return { id: r.id, kind: r.kind, target: r.target, status: r.status, createdAt: r.created_at, ...payload }
              }),
            })
          }
