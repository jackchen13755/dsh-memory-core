 ...
 *   GET  /badge?sessionId=                待确认/提示词/技能等角标计数（带 sessionId 只数该会话）
 *   GET  /suggestions?status=&kind=       待确认队列（sessionId 必带：面板只展示本会话）
 *   POST /suggestions/approve             { id|ids, overrides?, sessionId?, cwd? }（project/key 轨落当前项目 scope）
 *   POST /suggestions/reject              { id|ids, reason? }
 *   POST /suggestions/archive             { id|ids }
 *   POST /suggestions/restore             { id|ids }（归档放回待确认）
...
export function installApi(ctx, deps) {
  ...
  /**
   * 采纳一律走**按 kind 分派的统一入口**（记忆 / 待办 / 技能，装配见 lib/index.js）。
   *
   * 历史 bug：这里曾直连 `runtime.writer.approveSuggestion`（记忆专用），技能建议的载荷
   * （name/description/body，没有 content）被当成记忆体 → 判「建议内容为空」而失败，
   * 面板「采纳（写入技能库）」因此永远不生效：卡片还在、技能库不进、只闪一条失败提示。
   * `deps.approveSuggestion` 缺席时才退回记忆专用路径（老组合仍可用，绝不抛）。
   */
  const approveByKind =
    typeof deps.approveSuggestion === 'function' ? deps.approveSuggestion : (args) => runtime.writer.approveSuggestion(args)
      ...
      const handler = async (req, res) => {
        ...
        try {
          ...
          if (method === 'GET' && path === '/badge') {
            // 待确认计数默认全库；带 sessionId 时只数该会话（面板红点/页签计数要跟「本会话」列表一致，
            // 否则角标显示 12 条、点进去只有 3 条 —— 那是跨会话串味的老毛病）。
            const badgeSession = url.searchParams.get('sessionId')
            const scoped = badgeSession && badgeSession !== 'all' ? badgeSession : null
            const filter = scoped === 'orphan' ? ' AND session_id IS NULL' : scoped ? ' AND session_id = ?' : ''
            const args = scoped && scoped !== 'orphan' ? [scoped] : []
            const countSuggestions = (extra = '') =>
              Number(store.db.prepare(`SELECT COUNT(*) AS n FROM suggestions WHERE status = 'pending'${extra}${filter}`).get(...args)?.n ?? 0)
            ...
            return sendJson(res, 200, {
              ...
              todoSuggestions: countSuggestions(" AND kind = 'todo'"),
              skills: countSuggestions(" AND kind = 'skill'"),
          ...
          if (method === 'POST' && path.startsWith('/suggestions/')) {
            ...
            /**
             * 采纳 project/key 轨时的作用域 = **当前打开的项目**：按请求带来的 sessionId 反查
             * （活体会话 cwd 优先，其次 sessions.scope），调用方显式给了 cwd 才用 cwd 兜底。
             * 不这么做的话，跨会话/跨项目采纳会把条目写进建议产生时那个项目。
             */
            const approveSession = body.sessionId ?? url.searchParams.get('sessionId') ?? null
            const approveCwd = body.cwd ?? url.searchParams.get('cwd') ?? null
            const projectScope =
              runtime?.projectScopeForSession?.(approveSession) ?? (approveCwd ? `project:${projectHash(approveCwd)}` : null)
            const results = []
            for (const id of ids) {
              if (action === 'approve') {
                results.push({ id, ...approveByKind({ id, overrides: body.overrides ?? {}, sessionId: approveSession, projectScope }) })
              ...
              else if (action === 'restore') results.push({ id, ...runtime.writer.restoreSuggestion({ id }) })
