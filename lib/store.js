...
import { sessionIdFromOrigin } from './writer.js'
...
export class Store {
  ...
  /**
   * 回填建议归属会话（历史数据修复，幂等）。
   * 早期两条写入路径都没把归属会话落到 `session_id` 上，但 payload 里留着线索：
   * `origin` 写着 `extract:<会话>`（会话提取路径）或 `session:<会话>`（mem_write 等工具路径）；
   * 实在没有就退到 payload 自带的会话 id 字段。用它把 session_id 补回来，
   * 面板才能按会话区分待确认（否则这些历史条目会永远只出现在「全部」里）。
   */
  backfillSuggestionSessions() {
    ...
    for (const row of rows) {
      ...
      const sid = sessionIdFromOrigin(payload?.origin) ?? nonEmptyString(payload?.sessionId) ?? nonEmptyString(payload?.session_id)
      if (!sid) continue
      upd.run(sid, row.id)
      fixed += 1
    }
