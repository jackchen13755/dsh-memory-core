...
export class Store {
  ...
  listSuggestions({ status = 'pending', kind = null, limit = 200 } = {}) {
    const where = ['status = ?']
    const args = [status]
  ...
  listSkills({ enabled = null } = {}) {
    if (enabled === null) return this.db.prepare('SELECT * FROM skills ORDER BY name').all()
    return this.db.prepare('SELECT * FROM skills WHERE enabled = ? ORDER BY name').all(enabled ? 1 : 0)
  }
