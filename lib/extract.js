 ...
 *   - 结构化输出的 LLM 调用**必须显式声明推理档位**（默认 off），绝不继承宿主默认；
...
export const EXTRACT_DEFAULTS = {
  ...
  distill: { reasoningEffort: 'off', maxTokens: 8000, failVisible: true, degradeAfterFailures: 3 },
...
export async function callExtractLlm({ llm, route, request, config = EXTRACT_DEFAULTS, signal = null, log = () => {} }) {
  ...
  try {
    ...
    return { ok: true, text, effort: cfg.reasoningEffort, degraded: null }
  } catch (error) {
    const message = String(error?.message ?? error)
    if (/REASONING_EFFORT|reasoning effort/i.test(message)) {
      log(`模型不支持推理档位 ${cfg.reasoningEffort}，退回不带档位重试：${message}`)
      try {
        const text = await attempt(false)
        return { ok: true, text, effort: null, degraded: `不支持 ${cfg.reasoningEffort}，已退回默认档位` }
      } catch (retryError) {
        return { ok: false, error: String(retryError?.message ?? retryError), effort: null, degraded: null }
      }
    }
    return { ok: false, error: message, effort: cfg.reasoningEffort, degraded: null }
  }
}
...
export function recordExtractFailure(store, { reason, rawPrefix = null, config = EXTRACT_DEFAULTS }) {
  ...
  if (rawPrefix) store.setMeta('extract_last_raw_prefix', String(rawPrefix).slice(0, 500))
 ...
 * @param {{ store: any, live: Map<string,{lastSeen:number, seq:number, turns:number}>, config?: object, now?: number }} deps
 */
export function planAutoExtract({ store, live = new Map(), config = EXTRACT_DEFAULTS, now = Date.now() }) {
  ...
  for (const [sessionId, state] of live.entries()) {
    ...
    // 从未提取过时以"首次活动时间"为基准，否则一个刚开的会话会立刻命中 max-wait
    const baseline = row?.last_extract_at ?? row?.last_seen ?? now
