...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** 轨道中文名（值仍是英文键，只影响显示） */
    const TRACK_LABELS = { user: '用户档案', memory: '长期记忆', key: '项目关键记忆', project: '项目日志', daily: '每日日志' }
    const trackLabel = (t) => TRACK_LABELS[t] || t || '-'
    const POLL_MS = 30000
    const PAGE_SIZE = 20
    const PREVIEW_CHARS = 400 // 列表里先只渲染这么多字，长条目点「展开全文」再看（项目日志单条常几 KB）
    const STYLE_ID = 'dsh-memory-core-styles'
    /** 宿主路由的会话过滤哨兵：`orphan` = 只看没有会话归属的历史数据。 */
    const SCOPE_ORPHAN = 'orphan'

    /**
     * 面板一律按会话过滤要传给宿主路由的 sessionId。
     * 拿不到当前会话（面板挂在没有会话上下文的地方）时退成 `orphan`，
     * 而不是放任调用方省略参数 —— 省略就等于宿主的 `all`（全库），
     * 跨会话混看再采纳，条目就会落到别的会话/项目里去。
     */
    function sessionScopeOf(sessionId) {
      return sessionId ? encodeURIComponent(String(sessionId)) : SCOPE_ORPHAN
    }

    /** 角标查询串：带上当前会话，红点/页签计数才和「本会话」列表一致（不带 = 宿主按全库数）。 */
    function badgeQuery(sessionId) {
      return '?sessionId=' + sessionScopeOf(sessionId)
    }

    /**
     * 从会话列表快照里取「当前正在展示的会话」。
     *
     * ⚠️ 历史 bug（本插件最要命的一处）：这里曾读 `snapshot.current` —— 而 DSH 的会话列表快照
     * 只有 `{ ids, byId, phase, subagentsByParent, jobsBySession }`，**没有 current 字段**
     * （官方包里零处这么读）。于是它永远返回 null → 面板一律退化成 `orphan`：
     * 「本会话」永远空、条目全挤在「全部」里，采纳时也拿不到"当前打开的项目"。
     * 官方口径（ui-workspace 三处同款写法）：主视图正在展示的会话 = `retainedBy.mainView > 0`。
     */
    function currentSessionIdOf(list) {
      if (!list) return null
      const entries = Object.values(list.byId ?? {})
      const shown = entries.find((s) => ((s && s.retainedBy && s.retainedBy.mainView) ?? 0) > 0)
      return (shown && shown.id) || list.current || null
    }
...
`
    ...
    function MemoryView(props) {
      ...
      const [fullText, setFullText] = st(null) // 正在"展开全文"的条目 id
      ...
      const load = React.useCallback(async () => {
        ...
        const live = props.getSessionId ? props.getSessionId() : sessionId
        if (live) params.set('sessionId', live)
        try {
      ...
      // 上游（store.listUnits）已按 created_at DESC 返回 → 最新在前；这里只需分页，**不要再 reverse**
      const pageEntries = filtered === null ? null : filtered.slice(safePage * PAGE_SIZE, (safePage + 1) * PAGE_SIZE)
      ...
      const body = mode === 'raw'
        ...
        : pageEntries.map((entry) => h('div', { key: entry.id, className: 'memcore-entry' },
            ...
            editing === entry.id
              ...
              : (() => {
                  const text = entry.content || entry.snippet || ''
                  const isFull = fullText === entry.id || text.length <= PREVIEW_CHARS
                  return h('div', { className: 'memcore-entry-text' }, isFull ? text : text.slice(0, PREVIEW_CHARS) + '…')
                })(),
            editing === entry.id ? null : h('div', { className: 'memcore-entry-head' },
              h('span', { className: 'memcore-entry-ops' },
                (entry.content || entry.snippet || '').length > PREVIEW_CHARS
                  ? h(Btn, { onClick: () => setFullText(fullText === entry.id ? null : entry.id) }, fullText === entry.id ? '收起' : '展开全文')
                  : null,
    ...
    function QueueView(props) {
      ...
      const [counts, setCounts] = st({ session: 0, archived: 0 })
      // 提取产物是按会话来的：**只看本会话**（没有"全部"档 —— 跨会话混看再采纳，
      // 条目会落到别的会话/项目里）；「已归档」是归档动作的落脚点（从前端只发不收，
      // 归档后就再也找不到了），同样只列本会话。
      const [scope, setScope] = st('session')
      /** 会话 id 实时取：面板常驻，用户在 DSH 里切会话后过滤与采纳都要跟着变。 */
      const liveOf = () => (props.getSessionId ? props.getSessionId() : props.sessionId) ?? null
      const loadedFor = React.useRef(null)
      const load = async () => {
        const live = liveOf()
        const [s1, s3] = await Promise.all([
          api('/suggestions?status=pending&sessionId=' + sessionScopeOf(live)),
          api('/suggestions?status=archived&sessionId=' + sessionScopeOf(live)),
        ])
        const mineRows = s1.entries || []
        const archivedRows = s3.entries || []
        setCounts({ session: mineRows.length, archived: archivedRows.length })
        setItems(scope === 'session' ? mineRows : archivedRows)
      }
      React.useEffect(() => {
        loadedFor.current = liveOf()
        void load()
        // 30s 自刷：会话提取/工具写入随时可能往队列里放新条目，面板不重查就会"看着像没写进来"；
        // 会话 id 变了也立刻重查（否则会拿着上一个会话的队列让人采纳，落点必然不对）。
        const timer = setInterval(() => {
          const live = liveOf()
          if (live !== loadedFor.current) loadedFor.current = live
          void load()
        }, POLL_MS)
      ...
      if (items.length === 0) {
        return h('div', { className: 'memcore-entries' },
          h('div', { className: 'memcore-toolbar' },
            h('div', { className: 'memcore-seg', role: 'group' },
              h('button', { type: 'button', className: scope === 'session' ? 'memcore-seg-active' : '', onClick: () => setScope('session') }, '本会话 (' + counts.session + ')'),
              h('button', { type: 'button', className: scope === 'archived' ? 'memcore-seg-active' : '', onClick: () => setScope('archived') }, '已归档 (' + counts.archived + ')'))),
          h('div', { className: 'memcore-empty' }, liveOf() === null
            ? '拿不到当前会话 id（面板不在会话上下文里），没法按会话列出待确认 —— 请刷新页面或重开「记忆核心」页签。'
            : scope === 'session'
              ? '本会话暂无待确认。会话结束后会自动提取，产出先进这里；采纳后才会注入上下文。'
              : '本会话没有已归档的待确认。卡片上点「归档」的条目会落在这里，可点「恢复」放回队列。'))
      }
      // 三类建议的载荷不同：memory 有 content；skill 有 name/description/body；todo 有 content+track
      const headOf = (item) => (item.kind === 'skill' ? item.name || '(未命名技能)' : item.content || '(无内容)')
      const subOf = (item) => (item.kind === 'skill' ? item.description || '' : '')
      const acceptLabel = (item) => (item.kind === 'skill' ? '采纳（写入技能库）' : item.kind === 'todo' ? '采纳（落地待办）' : '采纳（可改轨）')
      let rows
      try {
        rows = items.map((item) => h('div', { key: item.id, className: 'memcore-entry' },
        h('div', { className: 'memcore-entry-head' },
          h(Badge, { track: item.track }, item.kind === 'skill' ? '技能建议' : (item.kind || 'memory') + ' · ' + trackLabel(item.track || item.target)),
          item.reason ? h(Badge, { cls: 'memcore-badge-ro' }, String(item.reason).slice(0, 24)) : null,
          h(Badge, { cls: 'memcore-badge-ro' }, item.id)),
        h('div', { className: 'memcore-entry-text' }, headOf(item)),
        subOf(item) ? h('div', { className: 'memcore-muted' }, subOf(item)) : null,
        h('div', { className: 'memcore-entry-head' },
          item.kind === 'skill' || item.kind === 'todo' || scope === 'archived'
            ? null
            : h('select', { className: 'memcore-select', defaultValue: item.track || 'memory', title: '采纳后写入哪条轨', onChange: (e) => { item.__track = e.target.value } }, TRACKS.map((t) => h('option', { key: t, value: t }, trackLabel(t)))),
          h('span', { className: 'memcore-entry-ops' },
            scope === 'archived'
              ? h(Btn, { kind: 'primary', disabled: props.busy, onClick: () => act(() => api('/suggestions/restore', { method: 'POST', body: { id: item.id } }), '已恢复（回到待确认队列）') }, '恢复')
              : h('span', { className: 'memcore-entry-ops' },
                  h(Btn, { kind: 'primary', disabled: props.busy, onClick: () => act(() => api('/suggestions/approve', { method: 'POST', body: { id: item.id, sessionId: liveOf(), overrides: { track: item.__track || item.track } } }), '已采纳') }, acceptLabel(item)),
        ...
        // 单条数据异常不该让整个面板消失：渲染成可见错误
        return h('div', { className: 'memcore-entries' },
          h('div', { className: 'memcore-notice memcore-notice-error' }, '待确认队列渲染失败：' + (e && e.message ? e.message : String(e))),
          h(Btn, { onClick: () => { props.setBusy(true); void load().finally(() => props.setBusy(false)) } }, '重试'))
      }
      return h('div', { className: 'memcore-entries' },
        h('div', { className: 'memcore-toolbar' },
          h('div', { className: 'memcore-seg', role: 'group' },
            h('button', { type: 'button', className: scope === 'session' ? 'memcore-seg-active' : '', onClick: () => setScope('session') }, '本会话 (' + counts.session + ')'),
            h('button', { type: 'button', className: scope === 'archived' ? 'memcore-seg-active' : '', onClick: () => setScope('archived') }, '已归档 (' + counts.archived + ')')),
          h('span', { className: 'memcore-count' }, scope === 'session' ? '只显示本会话的提取产物；采纳 project/key 轨会落到当前项目' : '归档不是终态：可点「恢复」放回待确认队列')),
        rows)
    }
    ...
    function MemoryPanel(props) {
      ...
      const pollCounts = React.useCallback(async () => {
        // 计数一律按**本会话**取：页签上的数字必须和「待确认」列表里的条数一致
        // （按全库数会出现"角标 12 条、点进去 3 条"，看着就像别的会话的产出串进来了）。
        const live = props.getSessionId ? props.getSessionId() : null
        const badge = await api('/badge' + badgeQuery(live))
        const pending = await api('/suggestions?status=pending&sessionId=' + sessionScopeOf(live))
      ...
      // 会话 id 在**请求时**实时取：面板常驻，用户在 DSH 里切换会话后"本会话"要跟着变
      const sessionId = props.getSessionId ? props.getSessionId() : props.sessionId ?? null
      ...
      return h('div', { className: 'memcore-panel' },
        ...
        h(
          ...
          feature === 'queue'
            ? h(QueueView, { onNotice: setNotice, busy, setBusy, onChanged: pollCounts, sessionId, getSessionId: props.getSessionId })
            : feature === 'todos'
              ...
              : feature === 'skills'
                ...
                : feature === 'prompts'
                  ...
                  : tab.kind === 'rules'
                  ? h(RulesView, null)
                  : h(MemoryView, { tab, sessionId, getSessionId: props.getSessionId, onNotice: setNotice, busy, setBusy, onCwd: setCwd }),
        ),
      )
    }
    ...
    function apply(ctx) {
      ...
      /** 当前会话 id（请求时实时取）：走官方口径 currentSessionIdOf，见上面的历史 bug 注释。 */
      const currentSessionId = () => {
        try {
          return currentSessionIdOf(ctx.sessions && ctx.sessions.list && ctx.sessions.list.getSnapshot())
        } catch (e) {
          return null
        }
      }
      ...
      const register = () => {
        ...
        disposeRegistration = ctx.slots.inject('conversation.view', () => ctx.slots.register({
          ...
          /**
           * 这个 view 挂在哪个会话就认哪个会话 —— 官方口径：slot 的 inject 第一个参数
           * 就是该 view 绑定的 session key（ui-trajectory 同款用法）。面板因此不必靠
           * "全局当前会话"猜：多会话同时打开时，A 的待确认不会被显示到 B 的面板里。
           * 万一宿主不给（老版本/非会话作用域），退空对象，面板再走 currentSessionId 兜底。
           */
          inject: (sessionKey) => {
            try {
              if (!sessionKey) return {}
              return ctx.sessions && ctx.sessions.binding && ctx.sessions.binding(sessionKey)
                ? { viewSessionId: String(sessionKey) }
                : {}
            } catch (e) {
              return {}
            }
          },
        }, (props) => h(MemoryPanel, (props && props.viewSessionId)
          ? { sessionId: props.viewSessionId }
          : { getSessionId: currentSessionId })))
      }

      const poll = async () => {
        try {
          // 红点也按当前会话数：跨会话的待确认不在面板里展示，就不该在角标上催人
          const next = await api('/badge' + badgeQuery(currentSessionId()))
    ...
    // 会话/作用域解析是纯函数，能被测到才不会又退化成"读过不存在的字段"。
    return { inject, apply, __test: { currentSessionIdOf, sessionScopeOf, badgeQuery } }
  },
})
