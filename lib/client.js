...
`
    ...
    function QueueView(props) {
      ...
      const [counts, setCounts] = st({ session: 0, all: 0 })
      ...
      const load = async () => {
        ...
        const mine = live ? encodeURIComponent(live) : 'orphan'
        const [s1, s2] = await Promise.all([
          api('/suggestions?status=pending&sessionId=' + mine),
          api('/suggestions?status=pending&sessionId=all'),
        ])
        const mineRows = s1.entries || []
        const allRows = s2.entries || []
        setCounts({ session: mineRows.length, all: allRows.length })
        setItems(scope === 'session' ? mineRows : allRows)
      }
      ...
      try {
        ...
        h('div', { className: 'memcore-entry-head' },
          ...
          scope === 'all' && item.sessionId && item.sessionId !== liveSession
        ...
        h('div', { className: 'memcore-entry-head' },
          item.kind === 'skill' || item.kind === 'todo'
            ? null
      ...
      return h('div', { className: 'memcore-entries' },
        h('div', { className: 'memcore-toolbar' },
          h('div', { className: 'memcore-seg', role: 'group' },
            h('button', { type: 'button', className: scope === 'session' ? 'memcore-seg-active' : '', onClick: () => setScope('session') }, '本会话 (' + counts.session + ')'),
            h('button', { type: 'button', className: scope === 'all' ? 'memcore-seg-active' : '', onClick: () => setScope('all') }, '全部 (' + counts.all + ')')),
          h('span', { className: 'memcore-count' }, scope === 'session' ? '只显示本会话的提取产物' : '跨会话汇总，卡片上标注来源会话')),
        rows)
    }
