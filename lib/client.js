...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** 宿主路由的会话过滤哨兵：`all` = 不按会话过滤；`orphan` = 只看没有会话归属的历史数据。 */
    const SCOPE_ALL = 'all'
    const SCOPE_ORPHAN = 'orphan'

    /**
     * 「本会话」视图要传给宿主路由的 sessionId。
     * 拿不到当前会话（面板挂在没有会话上下文的地方）时退成 `orphan`，
     * 而不是放任调用方省略参数 —— 省略就等于 `all`，会把别的会话的待确认串进「本会话」。
     */
...
`
    ...
    function QueueView(props) {
      ...
      const [counts, setCounts] = st({ session: 0, all: 0, archived: 0 })
      // 提取产物是按会话来的：默认只看本会话，切「全部」才跨会话汇总（并标出来源会话）；
      // 「已归档」是归档动作的落脚点（从前端只发不收，归档后就再也找不到了）。
      const [scope, setScope] = st('session')
      const load = async () => {
        const live = props.getSessionId ? props.getSessionId() : props.sessionId
        const [s1, s2, s3] = await Promise.all([
          api('/suggestions?status=pending&sessionId=' + sessionScopeOf(live)),
          api('/suggestions?status=pending&sessionId=' + SCOPE_ALL),
          api('/suggestions?status=archived&sessionId=' + SCOPE_ALL),
        ])
        const mineRows = s1.entries || []
        const allRows = s2.entries || []
        const archivedRows = s3.entries || []
        setCounts({ session: mineRows.length, all: allRows.length, archived: archivedRows.length })
        setItems(scope === 'session' ? mineRows : scope === 'all' ? allRows : archivedRows)
      }
      React.useEffect(() => { void load() }, [scope])
      const liveSession = props.getSessionId ? props.getSessionId() : props.sessionId
      const act = async (fn, okText) => {
        props.setBusy(true)
        try {
          const res = await fn()
        ...
        return h('div', { className: 'memcore-entries' },
          h('div', { className: 'memcore-toolbar' },
            h('div', { className: 'memcore-seg', role: 'group' },
              ...
              h('button', { type: 'button', className: scope === 'all' ? 'memcore-seg-active' : '', onClick: () => setScope('all') }, '全部 (' + counts.all + ')'),
          ...
          h('div', { className: 'memcore-empty' }, scope === 'session'
            ? (counts.all > 0 ? '本会话暂无待确认（其它会话有 ' + counts.all + ' 条，切「全部」查看）。' : '会话结束后会自动提取，产出先进这里；采纳后才会注入上下文。')
            : scope === 'all'
              ? '队列为空。会话结束后会自动提取，产出先进这里；采纳后才会注入上下文。'
              : '没有已归档的待确认。卡片上点「归档」的条目会落在这里，可点「恢复」放回队列。'))
      }
      ...
      try {
        ...
        h('div', { className: 'memcore-entry-head' },
          ...
          scope !== 'session' && item.sessionId && item.sessionId !== liveSession
            ? h(Badge, { cls: 'memcore-badge-ro' }, '来自 ' + String(item.sessionId).replace(/^session-/, '').slice(0, 8))
            : null,
          scope !== 'session' && !item.sessionId
            ? h(Badge, { cls: 'memcore-badge-ro' }, '无会话归属')
            : null,
        ...
        h('div', { className: 'memcore-entry-head' },
          ...
          h('span', { className: 'memcore-entry-ops' },
            scope === 'archived'
              ...
              : h('span', { className: 'memcore-entry-ops' },
                  h(Btn, { kind: 'primary', disabled: props.busy, onClick: () => act(() => api('/suggestions/approve', { method: 'POST', body: { id: item.id, overrides: { track: item.__track || item.track } } }), '已采纳') }, acceptLabel(item)),
      ...
      return h('div', { className: 'memcore-entries' },
        h('div', { className: 'memcore-toolbar' },
          h('div', { className: 'memcore-seg', role: 'group' },
            ...
            h('button', { type: 'button', className: scope === 'all' ? 'memcore-seg-active' : '', onClick: () => setScope('all') }, '全部 (' + counts.all + ')'),
            h('button', { type: 'button', className: scope === 'archived' ? 'memcore-seg-active' : '', onClick: () => setScope('archived') }, '已归档 (' + counts.archived + ')')),
          h('span', { className: 'memcore-count' }, scope === 'session' ? '只显示本会话的提取产物' : scope === 'all' ? '跨会话汇总，卡片上标注来源会话' : '归档不是终态：可点「恢复」放回待确认队列')),
        rows)
    }
